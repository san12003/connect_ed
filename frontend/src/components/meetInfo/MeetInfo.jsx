...
function MeetInfo({data, onClose, onDelete}) {
    ...
    const handleResendEmail = async () => {
        try {
            ...
            const result = await response.json().finally(() => setIsLoading(false));
            if (response.ok) {
    ...
    return (
        <>
            ...
            <div className={styles.cardContainer}>
                ...
                <button onClick={()=>navigate('/videoCall', { state: { currentLink: data.link } })}  style={{background:"var(--primary_color)" , color:"var(--tertiary_color)" , width:"95%" ,height:"150%", marginLeft: "1rem" , borderRadius: "0.5rem" , border:"none"}}>
